...
public class FlagValidator {
    ...
    // Only the Reverse Engineering lesson retains a local hash for offline/introductory use.
    // All other modules require server-side validation — no hashes are shipped in the APK.
    ...
    public static boolean validateFlag(Module module, String submittedFlag) {
        ...
        Log.d(TAG, isValid ? "✓ Correct flag for " + module.getId()
                             : "✗ Incorrect flag for " + module.getId());
        return isValid;
    }
    ...
    public static void validateFlag(
        ...
        if (serverUrl.isEmpty() || login.isEmpty() || sessionCookie.isEmpty()) {
            // No active server session — fall back to local validation (only RE_LESSON has a hash)
            Log.d(TAG, "No server session, using local validation for " + module.getId());
